...
def orf(seq):
    frames=[]
    frames.append([cdna[i:i + 3] for i in range(0,len(cdna),3)])
    frames.append([cdna[i:i + 3] for i in range(1,len(cdna),3)])
    frames.append([cdna[i:i + 3] for i in range(2,len(cdna),3)])
...
dnaseq=input ("enter the DNA seq:(use only A,T,G and C)\n")
...
proteinseq1= gen1(cdna)
print("Protein sequence of gen1:\n",proteinseq1)


print("Open Reading Frames for DNA are:\n\n",orf(cdna))
